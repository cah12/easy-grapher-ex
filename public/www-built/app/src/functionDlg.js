  ...
  constructor(plot) {
    ...
    var m_dlg1 = $(
      '\
                ...
                <div class="col-sm-3"><input id="fnDlg_lowerLimitY" style="width:100%" type="text" value="-10.0"/></div>\
                <div class="col-sm-3">Upper limit(y):</div>\
                <div class="col-sm-3"><input id="fnDlg_upperLimitY" style="width:100%" type="text" value="10.0"/></div>\
                </div>\
                <!--br-->\
                <div id="limitsFxy" class="row">\
                <div class="col-sm-3">Lower limit(f(xy)):</div>\
                <div class="col-sm-3"><input id="fnDlg_lowerLimitFxy" style="width:100%" type="text" value="0"/></div>\
                <div class="col-sm-3">Upper limit(f(xy)):</div>\
                <div class="col-sm-3"><input id="fnDlg_upperLimitFxy" style="width:100%" type="text" value="10.0"/></div>\
                </div>\
                <!--br-->\
      ...
      function negativeRootFn() {
        ...
        for (let i = 0; i < filtered.length; i++) {
          ...
          if (filtered[i].fn && filtered[i].fn.name === "sqrt") {
            const s = self.expandedFn.replace(
        ...
        if ($("#fnDlg_numberOfPoints").val() < 2) {
          ...
          try {
            self.upperLimit = math.evaluate(
              replaceParameterWith_1(
                plot.defines.expandDefines($("#fnDlg_upperLimit").val())
              )
            );
            if (!self.upperLimit) {
          ...
          if (!self.threeD) {
            ...
            try {
              self.lowerLimitY = math.evaluate($("#fnDlg_lowerLimitY").val());
            } catch (err) {
              Utility.alert("Please enter a valid lower(y) limit.");
              return false;
            }
            try {
              self.upperLimitY = math.evaluate($("#fnDlg_upperLimitY").val());
            } catch (err) {
              Utility.alert("Please enter a valid upper(y) limit.");
              return false;
            }
            try {
              self.lowerLimitFxy = math.evaluate(
                $("#fnDlg_lowerLimitFxy").val()
              );
            } catch (err) {
            ...
            try {
              self.upperLimitFxy = math.evaluate(
                $("#fnDlg_upperLimitFxy").val()
              );
            } catch (err) {
