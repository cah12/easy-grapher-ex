...
class MyPlot extends Plot {
  constructor(plotDiv, plotTitle) {
    ...
    Static.bind("addSpectrogram", function (e, fileName, samples, upload) {
      addSpectrogram(fileName, "#008b8b", "#ff0000", samples, upload);
    });
      ...
      if (operationType == "Discontinuity point") {
        ...
        const discontinuity = curves[0].discontinuity.map(function (e) {
          return Utility.toPrecision(
            Utility.adjustForDecimalPlaces(e, decimalPlacesX),
            precisionX
          );
        });
      ...
      if (operationType == "Intersection") {
        ...
        if (curves.length > 1) {
          ...
          if (curves[0].expandedFn && curves[1].expandedFn) {
            Utility.logStep("info", `\nFinding the intersection of "${curves[0].expandedFn}" and "${curves[1].expandedFn}"`);
            Utility.logStep("info", `At any point of intersection\n\t${curves[0].expandedFn} = ${curves[1].expandedFn}`);
            const simplifiedExp = math
              .simplify(`(${curves[0].expandedFn})-(${curves[1].expandedFn})`)
              .toString();
            Utility.logStep("info", `Re-arranging and simplifying, we get\n\t${simplifiedExp} = 0`);
            ...
            if (solution && solution.length < 20) {
              ...
              Utility.logStep("solve", {solutions:solution, variable: curves[0].variable});
    ...
    const fsOptions = {
      ...
      listOfFileTypes: [
        ...
        { display: "Defines (*.def)", ext: ".def", defaultFilename: "Defines" },
    ...
    const fileSystemServices = (self.fileSystemServices = new FileServices(
      fsOptions
    ));
    ...
    class GrapherEditor extends Editor {
      ...
      getData() {
        //this.m_data.m_fs.hasModifiedFile = false;
