  ...
  Load\
    ...
    this.doAdd = function (name, value) {
      ...
      if (define) {
        ...
        let _name = name.replace("(", "openPar").replace(")", "closePar");
        //self.showDlg();
    ...
    this.addHandler("definesRemove", "click", function () {
      ...
      id = "#" + id.replace("(", "openPar").replace(")", "closePar");
    ...
    function doExpandDefines(str) {
      ...
      m_defines.forEach(function (value, key, map) {
        if (key.indexOf("(") == -1) {
          ...
          const m_res = str.slice();
          for (let i = 0; i < names.length; i++) {
            if (names[i].indexOf(f) != -1) {
              let n = 0;
              while (res.indexOf(f) !== -1 && n < 100) {
                n++;
                ...
                let s_par = 0;
                let o_par = false;
                for (let index = 0; index < m_res.length; index++) {
...
*/
      if (m_simplify) {
        try {
          res = math
            .simplify(
              Utility.replaceKeywordMarkers(res),
              {},
...
class MDefines extends Defines {
  constructor(plot, editor) {
    ...
    $(window).bind("fileOpened", function (e, data, filename, ext, editorName) {
      ...
      if (editorName === self.plot.definesEditor.m_data.name) {
        const result = self.processUploadData({
          fileName: filename,
          content: data,
        });
