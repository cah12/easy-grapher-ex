...
class Utility {
  ...
  static curveInflectionPoint(
    ...
    let m_fn = fn;
    let result = [];
    let derivative = math.derivative(m_fn, variable);
  ...
  static logStep(type, obj = null) {
    switch (type) {
      ...
      case "solve":
        let solutionInfo = "";
        const { solutions, variable } = obj;
        for (let i = 0; i < solutions.length; i++) {
          solutionInfo += `\n\t${variable} = ${solutions.at(i).valueOf()}`;
          if (
            !isNaN(parseFloat(solutions.at(i).valueOf())) ||
            !isFinite(parseFloat(solutions.at(i).valueOf()))
          ) {
            solutionInfo += ` = ${math.evaluate(solutions.at(i).valueOf())}`;
          }
        }
        console.log(
          `Solving the equation for "${variable}" yields ${solutionInfo}`
        );
        break;
  ...
  static replaceLatex(result) {
    ...
    function purgeLatexFrac(str) {
      ...
      while (index !== -1) {
        // let ind = str.indexOf("^");
        // while (ind !== -1) {
        //   const prefix = Utility.getExponentTokenPrefix(str, ind);
        //   let obj = Utility.getOperandOfExponentToken(str, ind);
  ...
  static extendGetValue(mf) {
    ...
    mf.getValue = function (format = "latext") {
      const mf = this;

      const latex = mf.getValueTemp();
      let result = latex
        .replace(/\\times/g, "\\cdot")
